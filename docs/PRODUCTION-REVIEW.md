...
3. Complete [Google consent/OAuth setup](GOOGLE-CALENDAR.md). The current origin is `https://candidate.spaceone.tech`; its exact callback is `/api/calendar/google/callback`. Register localhost separately if testing locally, and make local APP_ORIGIN match the browser port.
4. Back up D1, apply `npm run db:remote`, then deploy with `npm run deploy`. Migration 0003 is additive. For Workers Builds, either use build `npm run check && npm run build` plus deploy `npx wrangler deploy`, or leave build empty and use deploy `npm run deploy`.
...
No production database migration, deployment, Google account authorization, or domain/DNS change was performed during this review.
