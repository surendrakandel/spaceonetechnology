...
<div class="overflow-clip">
	<header class="site-width pb-12 pt-9 lg:pb-16 lg:pt-12">
		<a
			class="mb-10 inline-flex items-center gap-2 text-[11px] text-muted hover:text-rust lg:mb-14"
		...
		>
			<div>
				...
				<h1 class="site-title">
					{titleLead}{#if accent}<span class="site-serif text-forest">{accent}</span>{/if}
				</h1>
				<p class="mb-0 max-w-[730px] text-base leading-8 text-muted sm:text-lg">{p.intro}</p>
				{#if isService}<a
						class="site-button mt-8 bg-forest text-white hover:bg-ink"
				...
				{#if p.kind === 'careers'}<div class="mt-8 flex flex-wrap items-center gap-6">
						<a
							class="site-button bg-forest text-white hover:bg-ink"
				...
				{#if p.kind === 'article'}<div class="mt-7 flex items-center gap-3 text-[11px] text-muted">
						<span class="flex h-7 w-7 items-center justify-center rounded-full bg-sage text-forest"
							><Asterisk size={16} /></span
						>Space One field notes<span aria-hidden="true">/</span>{readingTime} min read
					</div>{/if}
			</div>
			{#if p.kind === 'services'}<div
					class="hidden justify-self-end border-l border-line py-2 pl-8 lg:block"
				>
					<Asterisk size={45} strokeWidth={1} class="mb-5 text-rust" />
					<p class="mb-0 max-w-52 text-sm leading-7 text-muted">
						Bring the challenge.<br /><span class="font-medium text-ink"
				...
				>
					<div class="flex -space-x-3" aria-hidden="true">
						{#each media.avatars.slice(0, 4) as avatar}<img
								...
								width="52"
								height="52"
								class="h-12 w-12 shrink-0 rounded-full border-[3px] border-canvas bg-sage object-cover"
							/>{/each}
					</div>
					<p class="mb-0 max-w-52 text-xs leading-6 text-muted">
						{p.slug === 'about'
	...
	{#if p.kind === 'services'}
		<section class="border-y border-line bg-paper py-8 lg:py-12" aria-label="Technology services">
			<div class="site-width grid gap-x-12 md:grid-cols-2 lg:gap-x-20">
				{#each services as service, i}<a
						href={`/services/${service.slug}`}
						class="group border-b border-line py-8 last:border-0 md:last:border-b lg:py-10"
						><div class="mb-5 flex items-center justify-between">
							<span class="text-[11px] text-rust"
						...
						<h2 class="section-title group-hover:text-forest">{service.name}</h2>
						<p class="mb-5 max-w-xl text-sm leading-7 text-muted">{service.intro}</p>
						<span class="text-xs font-medium underline decoration-rust/30 underline-offset-8"
		...
		<section class="site-width grid gap-9 py-14 lg:grid-cols-[1fr_1fr] lg:py-20">
			<div>
				...
				<h2 class="site-heading">
					You don’t need<br /><span class="site-serif text-forest">a perfect brief.</span>
				</h2>
			</div>
			<div>
	...
	{:else if isService}
		<section class="border-y border-line bg-paper py-12 lg:py-16">
			<div class="site-width grid items-start gap-10 lg:grid-cols-[.6fr_1.4fr] lg:gap-20">
				<aside class="lg:sticky lg:top-8">
					...
					<h2 class="site-heading">
						{approach[0]}<br /><span class="site-serif text-forest">{approach[1]}</span>
					</h2>
				...
				<div>
					{#each p.sections as section, i}<section
							id={`section-${i + 1}`}
							class="scroll-mt-8 border-b border-line py-8 first:pt-0 last:border-0 last:pb-0"
						>
							<span class="mb-4 block text-[11px] text-rust">0{i + 1}</span>
		...
		<section class="site-width py-14 lg:py-20">
			<div class="grid gap-9 rounded-lg bg-sage p-7 sm:p-10 lg:grid-cols-[1fr_1fr]">
				<div>
					<p class="site-kicker">Before the work begins</p>
					<h2 class="mb-4 text-3xl font-medium tracking-tight">
						Everyone should know<br /><span class="site-serif text-forest">what happens next.</span>
					</h2>
					<p class="mb-0 max-w-md text-sm leading-7 text-muted">
				...
				<div class="self-center">
					{#each pads as pad, i}<div
							class="flex items-center justify-between border-b border-forest/15 py-4 text-sm first:pt-0 last:border-0"
						>
							<span><span class="mr-5 text-xs text-rust">0{i + 1}</span>{pad}</span><Check
								size={16}
								class="text-forest"
							/>
						</div>{/each}
			...
			<div class="mt-12 flex flex-wrap gap-6">
				<span class="text-xs text-muted">Related expertise</span>{#each services
					.filter((_, i) => i !== serviceIndex)
					.slice(0, 3) as service}<a
						class="text-xs font-medium hover:text-rust"
			...
			<aside class="order-2 lg:order-1">
				<div class="border-t border-line pt-7">
					...
					<h2 class="section-title">
						A project. A person.<br /><span class="site-serif text-forest"
				...
				<a class="my-7 inline-flex items-center gap-3 text-lg font-medium" href="tel:+18327105930"
					...
					>+1 (832) 710-5930</a
				>
				<p class="mb-7 text-xs text-muted">Monday–Friday · 8am–5pm Central</p>
				<div class="border-t border-line py-7">
					<h3 class="mb-3 text-sm font-medium">Our Irving office</h3>
					<address class="text-sm not-italic leading-7 text-muted">
				...
				<div class="rounded-lg bg-sand p-6">
					<h3 class="mb-3 text-base font-medium">Looking for your next role?</h3>
					<p class="mb-4 text-sm leading-7 text-muted">
	...
	{:else if p.kind === 'careers'}
		<section class="border-y border-line bg-paper py-12 lg:py-16">
			<div class="site-width">
				<div class="mb-10 flex items-center justify-between border-b border-line pb-6">
					<p class="site-kicker mb-0">From the first application to the next conversation</p>
					<Asterisk size={27} strokeWidth={1.2} class="ml-4 shrink-0 text-rust" />
				</div>
				<div class="grid gap-10 md:grid-cols-3">
					{#each p.sections as section, i}<section>
							<span
								class="mb-6 flex h-11 w-11 items-center justify-center rounded-full border border-forest/20 text-xs text-forest"
								>0{i + 1}</span
							>
							<h2 class="section-title text-xl">{section.title}</h2>
							<p class="mb-0 text-sm leading-7 text-muted">{section.body}</p>
						</section>{/each}
				</div>
			</div>
		</section>
		...
		>
			<figure class="m-0 overflow-hidden rounded-tl-[90px] rounded-br-[90px] bg-sage">
				<img
			...
			<div>
				...
				<h2 class="site-heading">
					Keep your energy<br /><span class="site-serif text-forest">for the opportunity.</span>
				</h2>
				{#each candidateFeatures as { icon: Icon, title, body }}<div
						class="flex gap-4 border-b border-line py-5 last:border-0"
					>
						<Icon size={20} strokeWidth={1.4} class="mt-1 shrink-0 text-rust" />
						<div>
							<h3 class="mb-2 text-base font-medium">{title}</h3>
							<p class="mb-0 text-sm leading-7 text-muted">{body}</p>
						</div>
					</div>{/each}
			</div>
		</section>
		<section class="site-width pb-16 lg:pb-24">
			<div
				class="flex flex-col justify-between gap-6 border-y border-line py-8 sm:flex-row sm:items-center"
			>
				<div>
					<h2 class="mb-2 text-2xl font-medium tracking-tight">
						The next interview starts with preparation.
					</h2>
					<p class="mb-0 text-sm text-muted">Ten steps to help you show your experience clearly.</p>
				</div>
	...
	{:else if p.kind === 'articles'}
		<section class="site-width pb-16 lg:pb-24" aria-label="Field notes">
			<a
				...
				class="group grid overflow-hidden rounded-lg border border-line bg-paper md:grid-cols-2"
				><img
				...
				<div class="flex flex-col justify-center p-7 lg:p-12">
					...
					<h2 class="site-heading text-3xl group-hover:text-forest">{articles[1].title}</h2>
					<p class="mb-7 text-sm leading-7 text-muted">{articles[1].intro}</p>
			...
			<div class="mt-12 grid gap-x-8 gap-y-12 md:grid-cols-2 lg:grid-cols-3">
				{#each articles.filter((_, i) => i !== 1) as article, i}<a
						href={`/insights/${article.slug}`}
						class="group border-t border-line pt-6"
						><div class="mb-8 flex items-center justify-between">
							<span class="text-[10px] uppercase tracking-[.14em] text-rust"
								>Field note / {String(i + 1).padStart(2, '0')}</span
							><ArrowUpRight size={18} />
						</div>
						<h2 class="section-title text-xl group-hover:text-forest">{article.title}</h2>
		...
		<section class="site-width pb-16 lg:pb-24" aria-label="Product concepts">
			<div class="grid gap-x-8 gap-y-12 md:grid-cols-2">
				{#each projectIdeas as project, i}<a href={`/projects/${project.slug}`} class="group"
						><div class="relative mb-5 overflow-hidden rounded-lg bg-sage">
							<img
							...
							/><span
								class="absolute bottom-4 left-4 rounded bg-paper px-3 py-2 text-[10px] font-medium uppercase tracking-[.13em]"
						...
						<h2 class="mb-2 text-3xl font-medium tracking-tight group-hover:text-forest">
							{project.name}
						</h2>
						<p class="mb-0 max-w-md text-sm leading-7 text-muted">{project.line}</p></a
					>{/each}
			</div>
		</section>
	{:else if p.slug === 'about'}
		<section class="site-width pb-16 lg:pb-24">
			<figure
				class="relative m-0 overflow-hidden rounded-tl-[70px] rounded-br-[70px] bg-sage sm:rounded-tl-[110px] sm:rounded-br-[110px]"
			>
				<img
					src={media.collaboration}
				...
				/>
				<figcaption
					class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/80 to-transparent px-7 pb-8 pt-20 text-white sm:px-10"
				>
					<p class="mb-1 text-[10px] uppercase tracking-[.16em] text-white/75">
			...
			<div class="mt-14 grid gap-10 lg:grid-cols-[.55fr_1.45fr] lg:gap-20">
				<div>
					...
					<h2 class="site-heading">
						The details<br /><span class="site-serif text-forest">matter to us.</span>
					</h2>
				</div>
				<div>
					{#each p.sections as section, i}<section
							id={`section-${i + 1}`}
							class="scroll-mt-8 border-b border-line py-7 first:pt-0 last:border-0"
						>
							<div class="flex items-start gap-5">
								<span class="pt-2 text-[11px] text-rust">0{i + 1}</span>
								<div>
									<h2 class="section-title">{section.title}</h2>
									<p class="reading-copy mb-0">{section.body}</p>
								</div>
			...
			<div class="mt-10 grid gap-5 sm:grid-cols-2">
				<a
					class="group flex items-center justify-between gap-4 rounded-lg bg-sage p-7"
					href="/company/team"
					><span
						><span class="mb-2 block text-[10px] uppercase tracking-[.15em] text-forest"
				...
				><a
					class="group flex items-center justify-between gap-4 rounded-lg bg-sand p-7"
					href="/engagements"
					><span
						><span class="mb-2 block text-[10px] uppercase tracking-[.15em] text-rust"
	...
	{:else if p.slug === 'company/team'}
		<section class="border-y border-line bg-paper py-12 lg:py-16">
			<div class="site-width grid gap-x-14 gap-y-10 md:grid-cols-2">
				{#each p.sections as section, i}<section
						id={`section-${i + 1}`}
						class="scroll-mt-8 border-t border-line pt-7"
					>
						<div class="mb-7 flex items-center justify-between">
							<span class="text-[11px] text-rust">0{i + 1}</span><Asterisk
								size={25}
								strokeWidth={1}
								class="text-forest"
							/>
		...
		<section class="site-width grid gap-8 py-14 md:grid-cols-2 lg:py-20">
			<h2 class="site-heading">
				A shared outcome.<br /><span class="site-serif text-forest">Clear responsibilities.</span>
			</h2>
			<div>
				<p class="reading-copy">
		...
		<section class="site-width pb-16 lg:pb-24">
			<div class="grid gap-5 lg:grid-cols-3">
				{#each p.sections.slice(0, 3) as section, i}<section
						id={`section-${i + 1}`}
						class="flex scroll-mt-8 flex-col rounded-lg border border-line p-7 sm:p-9"
						class:bg-forest={i === 0}
						class:text-white={i === 0}
						class:bg-paper={i !== 0}
					>
						<span
							class="mb-10 block text-[11px]"
							class:text-sage={i === 0}
							class:text-rust={i !== 0}
							>0{i + 1} / {['DEFINED OUTCOME', 'CONTINUED CARE', 'ADDED EXPERTISE'][i]}</span
						>
						<h2 class="section-title">{section.title}</h2>
						<p class={`mb-8 text-sm leading-7 ${i === 0 ? 'text-white/75' : 'text-muted'}`}>
							{section.body}
						</p>
						<a
							class={`mt-auto inline-flex items-center justify-between gap-4 border-t pt-5 text-xs font-medium ${i === 0 ? 'border-white/20' : 'border-line'}`}
			...
			<section
				id="section-4"
				class="mt-12 grid scroll-mt-8 gap-7 border-y border-line py-8 md:grid-cols-[.8fr_1.2fr]"
			>
	...
	{:else}
		{#if p.kind === 'project'}<div class="site-width">
				<figure class="m-0 overflow-hidden rounded-lg bg-sage">
					<img
				...
				<p class="mb-0 mt-4 text-xs leading-6 text-muted">
		...
		<section
			class="site-width grid items-start gap-9 border-t border-line pb-16 pt-10 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-20 lg:pb-24 lg:pt-14"
		>
			<aside class="lg:sticky lg:top-8">
				<details open class="rounded-lg border border-line p-5 lg:border-0 lg:p-0">
					<summary
						class="cursor-pointer text-[10px] font-semibold uppercase tracking-[.15em] text-rust"
						>{legal
							? 'In this document'
					...
					>
					<nav aria-label="Page sections" class="mt-5 grid gap-4 text-xs leading-6 text-muted">
						{#each p.sections as section, i}<a href={`#section-${i + 1}`} class="hover:text-rust"
								>{section.title}</a
							>{/each}
					</nav>
				</details>
			...
			<article class="min-w-0 max-w-[740px]">
				{#each p.sections as section, i}<section
						id={`section-${i + 1}`}
						class="scroll-mt-8 border-b border-line pb-9 pt-9 first:pt-0 last:border-0 last:pb-0"
					>
						<h2 class="section-title">{section.title}</h2>
						<p class="reading-copy mb-0">{section.body}</p>
					</section>{/each}{#if p.kind === 'article'}<div class="mt-12 rounded-lg bg-sage p-7">
						...
						class="mt-12 rounded-lg bg-sage p-7"
					>
		...
		{#if p.kind === 'article'}<section class="border-t border-line bg-paper py-12 lg:py-16">
				<div class="site-width">
					<p class="site-kicker">Keep reading</p>
					<div class="grid gap-8 md:grid-cols-3">
						{#each articles.filter((_, i) => i !== articleIndex).slice(0, 3) as article}<a
								href={`/insights/${article.slug}`}
								class="group border-t border-line pt-5"
								><h2
									class="mb-4 text-xl font-medium leading-snug tracking-tight group-hover:text-forest"
								>
									{article.title}
								</h2>
								<span class="inline-flex items-center gap-3 text-xs text-muted"
